...
\author{
    ...
    \IEEEauthorblockA{
        ...
        \textit{COMSATS University Islamabad, Islamabad, Pakistan} \\
...
\begin{table}[htbp]
\centering
\caption{Dataset Partition Distribution and Target Score Statistics}
\label{tab:dataset_stats}
\begin{tabular}{lcccc}
\toprule
\textbf{Partition} & \textbf{Sample Count ($N$)} & \textbf{Mean Score ($\mu$)} & \textbf{Std Dev ($\sigma$)} & \textbf{Score Range} \\
\midrule
Training Set       & 5,099 (80.0\%)              & 47.21                       & 24.95                       & $[18.30, 90.70]$ \\
Evaluation Set     & 1,275 (20.0\%)              & 47.12                       & 25.04                       & $[18.45, 90.55]$ \\
\textbf{Full Corpus} & \textbf{6,374 (100.0\%)}  & \textbf{47.19}              & \textbf{24.97}              & $\mathbf{[18.30, 90.70]}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Pearson Correlation Coefficients with Ground-Truth ATS Score}
\label{tab:correlation}
\begin{tabular}{lcc}
\toprule
\textbf{Extracted Signal Modality} & \textbf{Feature Metric} & \textbf{Pearson Correlation ($r$)} \\
\midrule
Dense Semantic Representation      & Cosine Similarity ($S_{\text{dense}}$) & \textbf{+0.462} \\
Symbolic Skill Ontology            & Skill Recall ($\text{Rec}_{\text{skill}}$) & \textbf{+0.418} \\
Symbolic Skill Ontology            & Jaccard Overlap ($J_{\text{skill}}$)     & \textbf{+0.389} \\
Syntactic Lexical Overlap          & TF-IDF Cosine ($S_{\text{lex}}$)         & +0.345 \\
Document Structural Complexity     & Length Ratio ($R_{\text{len}}$)          & +0.182 \\
Symbolic Skill Gap Penalty         & Missing Gap Ratio ($P_{\text{gap}}$)     & \textbf{-0.435} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Gini Impurity Feature Importance Ranking from Gradient Boosted Ensembles}
\label{tab:feature_importance}
\begin{tabular}{lccc}
\toprule
\textbf{Feature Name} & \textbf{Signal Category} & \textbf{Relative Importance} & \textbf{Permutation $\Delta R^2$} \\
\midrule
$S_{\text{dense}}$ (SBERT Cosine)     & Contextual Neural     & 34.8\% & -0.118 \\
$J_{\text{skill}}$ (Jaccard Index)    & Skill Ontology        & 22.4\% & -0.074 \\
$P_{\text{gap}}$ (Gap Penalty)        & Skill Ontology        & 16.1\% & -0.052 \\
$R_{\text{len}}$ (Length Ratio)       & Document Structure    & 11.2\% & -0.031 \\
$S_{\text{lex}}$ (TF-IDF Cosine)      & Syntactic Lexical     & 9.5\%  & -0.024 \\
$\text{TTR}_R$ (Lexical Diversity)    & Document Structure    & 6.0\%  & -0.012 \\
\bottomrule
\end{tabular}
\end{table}
...
% ═══════════════════════════════════════════════════════════════════════════
% References (Self-Contained IEEE Bibliography)
% ═══════════════════════════════════════════════════════════════════════════
\begin{thebibliography}{15}

\bibitem{panchasara2023}
H.~Panchasara, M.~Gupta, and P.~Sharma, ``AI based job recommendation system using BERT,'' in \emph{Proc. IEEE Int. Conf. Artif. Intell. Appl. (ICAIA)}, 2023, pp. 112--119.

\bibitem{zhu2021}
Y.~Zhu, X.~Zhao, J.~Liu, Y.~Zheng, X.~Zeng, J.~Tian, and R.~Yan, ``Reciprocal-constrained interpretable job recommendation,'' in \emph{Proc. AAAI Conf. Artif. Intell. (AAAI)}, vol.~35, no.~5, 2021, pp. 4673--4681.

\bibitem{mao2023}
Y.~Mao, Y.~Cheng, and Z.~Shi, ``A job recommendation method based on attention layer scoring characteristics and tensor decomposition,'' \emph{Appl. Sci.}, vol.~13, no.~8, p. 4912, 2023.

\bibitem{guan2024}
J.~Guan, Y.~Yang, Z.~Yang, H.~Zhu, X.~Li, and H.~Xiong, ``JobFormer: Skill-aware job recommendation with semantic-enhanced Transformer,'' in \emph{Proc. ACM Web Conf. (WWW)}, 2024, pp. 3145--3155.

\bibitem{han2024}
X.~Han, Y.~Zhu, H.~Hu, Z.~Qin, W.~X. Zhao, and H.~Zhu, ``Adapting job recommendations to user preference drift with behavioral-semantic fusion learning,'' in \emph{Proc. 30th ACM SIGKDD Conf. Knowl. Discovery Data Min. (KDD)}, 2024, pp. 982--993.

\bibitem{dataset2024}
0xnbk, ``Resume-ATS score dataset v1 (English),'' \emph{Hugging Face Datasets}, 2024. [Online]. Available: \url{https://huggingface.co/datasets/0xnbk/resume-ats-score-v1-en}

\bibitem{reimers2019}
N.~Reimers and I.~Gurevych, ``Sentence-BERT: Sentence embeddings using Siamese BERT-networks,'' in \emph{Proc. Conf. Empirical Methods Natural Lang. Process. (EMNLP)}, 2019, pp. 3982--3992.

\bibitem{chen2016}
T.~Chen and C.~Guestrin, ``XGBoost: A scalable tree boosting system,'' in \emph{Proc. 22nd ACM SIGKDD Int. Conf. Knowl. Discovery Data Min. (KDD)}, 2016, pp. 785--794.

\bibitem{ke2017lightgbm}
G.~Ke, Q.~Meng, T.~Finley, T.~Wang, W.~Chen, W.~Ma, Q.~Ye, and T.-Y. Liu, ``LightGBM: A highly efficient gradient boosting decision tree,'' in \emph{Adv. Neural Inf. Process. Syst. (NeurIPS)}, vol.~30, 2017, pp. 3146--3154.

\bibitem{prokhorenkova2018catboost}
L.~Prokhorenkova, G.~Gusev, A.~Vorobev, A.~V. Dorogush, and A.~Gulin, ``CatBoost: Unbiased boosting with categorical features,'' in \emph{Adv. Neural Inf. Process. Syst. (NeurIPS)}, vol.~31, 2018, pp. 6638--6648.
